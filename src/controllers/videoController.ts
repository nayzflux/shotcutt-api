...
export const createVideo: RequestHandler = async (req, res) => {
  ...
  const cuttingMargin = (req.query.cuttingMargin || "0") as string;

  if (!preset) return res.sendStatus(400);

  const cuttingMarginParsed = parseFloat(cuttingMargin) || 0;

  if (cuttingMarginParsed > 0.5) return res.sendStatus(400);
  ...
  processingService.processVideo(video, file.filename, cuttingMarginParsed);

  res
    .status(201)
    .json({ video: { cuttingMargin: cuttingMarginParsed, ...video } });
};
